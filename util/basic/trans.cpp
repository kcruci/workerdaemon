...
#include "trans.h"
#include "basic/csconv.h"

/*
int CTrans::STOI(const char* szValue)
{
	return szValue == NULL ? 0 : strtoul(szValue, NULL, 10);
}

int CTrans::STOI(const std::string& sValue)
{
	return strtoul(sValue.c_str(), NULL, 10);
}
*/
...
unsigned long CTrans::STOUL(const char* szValue)
{
	return strtoul(szValue, NULL, 10);
}

const std::string CTrans::ULTOS(unsigned long ulValue, std::streamsize nPrecision, std::streamsize nWide, char chFill, std::ios::fmtflags fmt)
{
	std::string sValue;
	std::stringstream strm;
	strm.setf(fmt);
	strm<<std::setprecision(nPrecision)<<std::setw(nWide)<<std::setfill(chFill)<<ulValue;
	strm>>sValue;
	return sValue;
}

const std::string CTrans::UTOS(unsigned int uiValue, std::streamsize nPrecision, std::streamsize nWide, char chFill, std::ios::fmtflags fmt)
{
	std::string sValue;
	std::stringstream strm;
	strm.setf(fmt);
	strm<<std::setprecision(nPrecision)<<std::setw(nWide)<<std::setfill(chFill)<<uiValue;
	strm>>sValue;
	return sValue;
}

uin_t CTrans::STOUIN(const char* szValue)
{
...
const std::string CTrans::UINTOS(uin_t iValue, std::streamsize nPrecision, std::streamsize nWide, char chFill, std::ios::fmtflags fmt)
{
	std::string sValue;
	std::stringstream strm;
	strm.setf(fmt);
