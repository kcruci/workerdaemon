...
public:
    ...
    */
	...
	/*!
	* \brief 解析并把字符串转化为无符号长整型数据
	*/
	static unsigned long STOUL(const char* szValue);

	/*!
	* \brief 把unsigned long 数据转化为字符串
	...
	*/
	static const std::string ULTOS(unsigned long ulValue, std::streamsize nPrecision=-1, std::streamsize nWide=-1, char chFill=' ', std::ios::fmtflags fmt=std::ios::fixed);

	/*!
	* \brief 把unsigned int 数据转化为字符串
	* \param[in] iValue: 待转换的数据
	* \param[in] nPrecision: 小数精度位数
	* \param[in] nWidth: 总位数
	* \param[in] nFill: 填充字符
	...
	* \return string 转换好的字符串
	*/
	static const std::string UTOS(unsigned int uiValue, std::streamsize nPrecision=-1, std::streamsize nWide=-1, char chFill=' ', std::ios::fmtflags fmt=std::ios::fixed);

	/*!
	* \brief 把整型数据转化为uin_t类型
	*/
	static uin_t STOUIN(const char* szValue);

	/*!
	* \brief 把uin_t数据转化为字符串
	* \param[in] iValue: 待转换的数据
	* \param[in] nPrecision: 小数精度位数
	* \param[in] nWidth: 总位数
	* \param[in] nFill: 填充字符
	...
	* \return string 转换好的字符串
	*/
	static const std::string UINTOS(uin_t iValue, std::streamsize nPrecision=-1, std::streamsize nWide=-1, char chFill=' ', std::ios::fmtflags fmt=std::ios::fixed);
